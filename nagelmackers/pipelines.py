...
class NagelmackersPipeline:

    # Database setup
    conn = sqlite3.connect('nagelmackers.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `nagelmackers`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM nagelmackers WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO nagelmackers (date, title, link, content)"
